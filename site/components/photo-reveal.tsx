...
import { getMotionProfile, gsap, scrollContainer } from '@/lib/animation';
  ...
  useLayoutEffect(() => {
    ...
    const reveal = () => {
      media.add({ reduce: '(prefers-reduced-motion: reduce)', all: 'all' }, () => {
        ...
        animation.fromTo(image, { opacity: 0.5, scale: reduced ? 1 : lite ? 1.04 : 1.18, xPercent: reduced || lite ? 0 : -2 }, { opacity: 1, scale: 1, xPercent: 0, duration: reduced ? 0.14 : lite ? 0.35 : 1.1, ease: 'power2.out', clearProps: 'transform,opacity' });
        if (!reduced && !lite) animation.fromTo(root, { clipPath: 'inset(0 0 8% 0)' }, { clipPath: 'inset(0 0 0% 0)', duration: 0.55, ease: 'power2.out', clearProps: 'clipPath' }, 0);
      });
    };
